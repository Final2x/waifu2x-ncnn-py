    ...
    if (vkdev) {
...

        {
            ...
            static ncnn::Mutex lock;
            {
                ...
                if (spirv.empty()) {
                    if (tta_mode)
                        compile_spirv_module(waifu2x_preproc_tta_comp_data, sizeof(waifu2x_preproc_tta_comp_data),
                                             net.opt, spirv);
                    else
                        compile_spirv_module(waifu2x_preproc_comp_data, sizeof(waifu2x_preproc_comp_data), net.opt,
                                             spirv);
                }
            }
...

        {
            ...
            static ncnn::Mutex lock;
            {
                ...
                if (spirv.empty()) {
                    if (tta_mode)
                        compile_spirv_module(waifu2x_postproc_tta_comp_data, sizeof(waifu2x_postproc_tta_comp_data),
                                             net.opt, spirv);
                    else
                        compile_spirv_module(waifu2x_postproc_comp_data, sizeof(waifu2x_postproc_comp_data), net.opt,
                                             spirv);
                }
            }
